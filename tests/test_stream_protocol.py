...
def _frame(mode: int, exchange: int, token: str = "99926000") -> bytes:
    packet = bytearray(139 if mode == 3 else 123 if mode == 2 else 51)
    packet[0] = mode
    packet[1] = exchange
...
def test_decodes_snap_quote_volume_and_open_interest() -> None:
...
@pytest.mark.parametrize(
    "frame",
    [_frame(1, 1)[:50], _frame(3, 2)[:138], _frame(4, 2), _frame(1, 9)],
)
